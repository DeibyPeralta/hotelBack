...
const vista = (schema) => __awaiter(void 0, void 0, void 0, function* () {
...
const maxhabitaciones = (schema) => __awaiter(void 0, void 0, void 0, function* () {
    const client = yield pool.connect();
    try {
        yield client.query(`SET search_path TO ${schema}`);
        const queryResult = yield client.query(`SELECT MAX(num_habitacion) AS max_num_habitacion FROM habitaciones;`);
        return {
            isError: false,
            data: queryResult.rows
        };
...
const addTablero = (interno, num_habitacion, hora_llegada, aseo, llamada, destino, fecha_llegada, schema) => __awaiter(void 0, void 0, void 0, function* () {
    const client = yield pool.connect();
    try {
        yield client.query(`SET search_path TO ${schema}`);
        const habitacionResult = yield client.query(`SELECT estado FROM habitaciones WHERE num_habitacion = $1`, [num_habitacion]);
        ...
        yield client.query(`UPDATE habitaciones SET estado = 'Ocupada' WHERE num_habitacion = $1`, [num_habitacion]);
        return {
            isError: false,
...
const habitaciones = (schema) => __awaiter(void 0, void 0, void 0, function* () {
    const client = yield pool.connect();
    try {
        yield client.query(`SET search_path TO ${schema}`);
        const queryResult = yield client.query(`select estado, num_habitacion, comentario from habitaciones order by num_habitacion;`);
        return {
            isError: false,
            data: queryResult.rows
        };
...
const editar_Habitaciones = (body, schema) => __awaiter(void 0, void 0, void 0, function* () {
    const client = yield pool.connect();
    try {
        yield client.query(`SET search_path TO ${schema}`);
        yield client.query(`UPDATE habitaciones SET estado = $1, comentario = $2
...
const addHabitaciones = (body, schema) => __awaiter(void 0, void 0, void 0, function* () {
    const client = yield pool.connect();
    try {
        // console.log('***** Añadiendo datos del tablero *****');
        yield client.query(`SET search_path TO ${schema}`);
        yield client.query(`INSERT INTO habitaciones (estado, num_habitacion, comentario)
...
const historialHabitaciones = (body, schema) => __awaiter(void 0, void 0, void 0, function* () {
    const client = yield pool.connect();
    try {
        yield client.query(`SET search_path TO ${schema}`);
...
const historial = (schema) => __awaiter(void 0, void 0, void 0, function* () {
    const client = yield pool.connect();
    try {
        yield client.query(`SET search_path TO ${schema}`);
        const queryResult = yield client.query(`
...
const updateHistorial = (body, schema) => __awaiter(void 0, void 0, void 0, function* () {
    const client = yield pool.connect();
    try {
        yield client.query(`SET search_path TO ${schema}`);
        const queryResult = yield client.query(`
...
const deleteHabitaciones = (num_habitacion, schema) => __awaiter(void 0, void 0, void 0, function* () {
    const client = yield pool.connect();
    try {
        yield client.query(`SET search_path TO ${schema}`);
        const queryResult = yield client.query(`delete from tablero where num_habitacion = $1`, [num_habitacion]);
        return {
            isError: false,
            data: queryResult.rows
        };
...
const editar_tablero = (body, schema) => __awaiter(void 0, void 0, void 0, function* () {
    const client = yield pool.connect();
    try {
        yield client.query(`SET search_path TO ${schema}`);
        yield client.query(`UPDATE tablero SET  hora_llegada = $1, aseo = $2, llamada = $3, destino = $4
            WHERE  num_habicion = $5 OR interno = $6`, [body.hora_llegada, body.aseo, body.llamada, body.destino, body.num_habitacion, body.interno]);
        return {
            isError: false,
            data: 'ok'
        };
...
const validateSocio = (cod_socio, schema) => __awaiter(void 0, void 0, void 0, function* () {
    const client = yield pool.connect();
    try {
        yield client.query(`SET search_path TO ${schema}`);
        const queryResult = yield client.query(`SELECT nombre FROM socios WHERE cod_socio = $1 OR LOWER(nombre) ILIKE '%' || LOWER($1) || '%'`, [cod_socio]);
        return {
            isError: false,
            data: queryResult.rows[0]
        };
...
const cuadre_caja = (body, schema) => __awaiter(void 0, void 0, void 0, function* () {
    const client = yield pool.connect();
    try {
        yield client.query(`SET search_path TO ${schema}`);
        const values = body
            .map((row, index) => `($1, $${index * 2 + 2}, $${index * 2 + 3})`)
            .join(", ");
        const flatValues = body.flat();
        const fechaHoraActual = new Date().toISOString();
        const query = ` INSERT INTO efectivoDia (fechaSistema, efectivo, value) VALUES ${values} `;
        yield client.query(query, [fechaHoraActual, ...flatValues]);
...
const flujoEfectivo = (schema) => __awaiter(void 0, void 0, void 0, function* () {
    const client = yield pool.connect();
    try {
        yield client.query(`SET search_path TO ${schema}`);
        const queryResult = yield client.query(`        
...
const totalEfectivo = (schema) => __awaiter(void 0, void 0, void 0, function* () {
    const client = yield pool.connect();
    try {
        yield client.query(`SET search_path TO ${schema}`);
        const queryResult = yield client.query(`select id, base, efectivoDia, total, fecha from historialEfectivo order by fecha DESC;`);
        return {
            isError: false,
            data: queryResult.rows[0]
        };
...
const efectivo = (body, schema) => __awaiter(void 0, void 0, void 0, function* () {
    const client = yield pool.connect();
    try {
        yield client.query(`SET search_path TO ${schema}`);
        ...
        const { rowCount } = yield client.query(`SELECT 1 FROM historialEfectivo WHERE turno = $1 AND fechaturno = CURRENT_DATE`, [body.turno]);
        if (rowCount === 0) {
            yield client.query(`INSERT INTO historialEfectivo (base, efectivoDia, total, usuario, turno, pagosdeldia, fechaturno)
                VALUES ($1, $2, $3, $4, $5, $6, CURRENT_DATE)`, [processedData.base, processedData.efectivoDelDia, processedData.total, body.usuario, body.turno, processedData.pagosRealizados]);
        }
        yield client.query('DELETE FROM efectivoDia WHERE id <> 1;');
        yield client.query(`UPDATE gastosdiarios SET historial = true WHERE historial = false;`);
        return {
            isError: false,
...
const historialcajaBase = (schema) => __awaiter(void 0, void 0, void 0, function* () {
    const client = yield pool.connect();
    try {
        yield client.query(`SET search_path TO ${schema}`);
        const queryResult = yield client.query(`SELECT id, base, efectivodia, total, TO_CHAR(fecha, 'DD-MM-YYYY') AS fecha, usuario, pagosdeldia as pagos, turno
...
const historialGraficos = (filtros = {}, schema) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        const queryResult = yield client.query(baseQuery, values);
        return {
            isError: false,
            data: queryResult.rows
        };
...
const habitacionesDisponibles = (schema) => __awaiter(void 0, void 0, void 0, function* () {
    const client = yield pool.connect();
    try {
        yield client.query(`SET search_path TO ${schema}`);
        const queryResult = yield client.query(`SELECT num_habitacion FROM habitaciones WHERE estado = 'Disponible' `);
        return {
            isError: false,
            data: queryResult.rows
        };
...
const insertGastosDiarios = (body, schema) => __awaiter(void 0, void 0, void 0, function* () {
    const client = yield pool.connect();
    try {
        yield client.query(`SET search_path TO ${schema}`);
        const queryResult = yield client.query(`
...
const getGastosDiarios = (schema) => __awaiter(void 0, void 0, void 0, function* () {
    const client = yield pool.connect();
    try {
        yield client.query(`SET search_path TO ${schema}`);
        const queryResult = yield client.query(`SELECT id, efectivodia, descripcion, usuario, historial, fecha FROM gastosdiarios ORDER BY fecha DESC;`);
        return {
            isError: false,
            data: queryResult.rows
        };
...
const totalGastosDiarios = (schema) => __awaiter(void 0, void 0, void 0, function* () {
    const client = yield pool.connect();
    try {
        yield client.query(`SET search_path TO ${schema}`);
        const queryResult = yield client.query(`SELECT SUM(efectivodia) AS total_actual FROM gastosdiarios WHERE historial = false;`);
        return {
            isError: false,
...
const internoPlaca = (interno, schema) => __awaiter(void 0, void 0, void 0, function* () {
    const client = yield pool.connect();
    try {
        yield client.query(`SET search_path TO ${schema}`);
        const queryResult = yield client.query(`SELECT cod_socio AS socio, placa
